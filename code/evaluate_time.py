...
if __name__ == '__main__':
    ...
    for pat_len in sorted(os.listdir(folder)):
        if not pat_len.isdigit():
            continue

        folds = sorted(os.listdir('%s/%s' % (folder, pat_len)))

        for fold in folds:
            fold = int(fold)

            sequence_db = build_seqDB('%s/%s/%d' % (folder, pat_len, fold))
            ...
            p = mp.Process(target=TRASE, args=(sequence_db, min_sup, min_size, max_gap,))
            p.start()

            # Wait to complete or until 1000s past
            ...
            time_record.append(('TRASE', int(pat_len), fold, '%.3f' % TRASE_time))
            ...
            p = mp.Process(target=gb.run(), args=(sequence_db, min_sup, min_size, max_gap))
            p.start()
            # Wait to complete or until 1000s past
            p.join(timeout)
            ...
            time_record.append(('GAP-BIDE', int(pat_len), fold, '%.3f' % GB_time))
